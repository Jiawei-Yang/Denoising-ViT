...
def main(args: argparse.Namespace):
    ...
    model = DVT.PretrainedViTWrapper(model_identifier=args.model, stride=args.stride_size)
    pos_h = (args.input_size[0] - model.patch_size) // args.stride_size + 1
    pos_w = (args.input_size[1] - model.patch_size) // args.stride_size + 1

    args.feat_dim = model.n_output_dims
    args.noise_map_height = pos_h
    args.noise_map_width = pos_w

    # extract the normalization layer from timm's transformation
    normalizer = model.transformation.transforms[-1]
    ...
    pretrained_vit = DVT.PretrainedViTWrapper(model_identifier=args.model, stride=args.stride_size)
    teacher_model = DVT.Denoiser(
    ...
    logger.info(f"Loaded denoiser from {args.denoiser_ckpt}")
    msg = teacher_model.load_state_dict(torch.load(args.denoiser_ckpt)["denoiser"], strict=False)
    missing_keys = msg.missing_keys
    unwanted_keys = msg.unexpected_keys
    missing_keys = {k for k in missing_keys if "vit" not in k}
    logger.info(f"Missing keys: {missing_keys}, unexpected keys: {unwanted_keys}")
    teacher_model.eval()
    ...
    if args.distributed:
        ...
        model.forward = model.module.get_intermediate_layers
    else:
        model.forward = model.get_intermediate_layers
    ...
    steps_per_epoch = len(train_dataset) // (args.batch_size * num_tasks)
    logger.info(f"steps_per_epoch: {steps_per_epoch}")
    if args.num_iterations is None:
        args.num_iterations = steps_per_epoch * args.num_epochs
    ...
    for step, (samples, _) in enumerate(
        ...
        with torch.no_grad():
            target_dict = teacher_model(samples, return_dict=True)
        pred_denoised_feats = model(samples)[0]
        pred_denoised_feats = pred_denoised_feats.permute(0, 2, 3, 1)
        l2_loss = F.mse_loss(pred_denoised_feats, target_dict["denoised_feats"])
        cosine_sim = F.cosine_similarity(pred_denoised_feats, target_dict["denoised_feats"], dim=-1)
